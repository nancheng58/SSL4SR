...
class FreqVocab(object):
    ...
    def __init__(self, user_to_list):
        ...
        self.special_tokens = ["[Preference_1]","[Preference_2]","[Preference_3]",
                               "[Preference_4]","[Preference_5]","[Preference_6]",
                               "[Preference_7]","[Preference_8]","[Preference_9]",
                               "[Preference_10]","[Preference_11]"] #"[Preference_2]","[Preference_3]",
        self.token_to_ids = {}  # index begin from 1

        #first items
        for token, count in self.counter.most_common(): # 按照fre进行编号,记录的是token对应的fre-id
...
#             self.token_to_ids[user] = len(self.token_to_ids) + 1
        ...
        self.id_to_tokens = {v: k for k, v in self.token_to_ids.items()} # 记录的是token对应的fre-id
        self.vocab_words = list(self.token_to_ids.keys()) # 记录的是都有哪些token
