...
## Required parameters
flags.DEFINE_string(
    "bert_config_file", '../data/Amazon-toys/bert_config_toys_64_p11.json',
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string(
    "train_input_file", '../data/Amazon-toys/toysdefault.train_p11_learning1_faiss.tfrecord',
    "Input TF example files (can be a glob or comma separated).")

flags.DEFINE_string(
    "test_input_file", '../data/Amazon-toys/toysdefault.test_p11.tfrecord',
    "Input TF example files (can be a glob or comma separated).")

flags.DEFINE_string("checkpointDir", 'checkpoint/toys_learning_finetune_p11/',
                    "The output directory where the model checkpoints will be written.")

flags.DEFINE_string("vocab_filename", '../data/Amazon-toys/toysdefault.vocab_p11', "vocab filename")

flags.DEFINE_string("user_history_filename", '../data/Amazon-toys/toysdefault.his_p11', "user history filename")

## Other parameters
flags.DEFINE_string("init_checkpoint", 'checkpoint/toys_learning_pretrain_p11/model.ckpt-200000', "Initial checkpoint.") #

flags.DEFINE_integer("max_seq_length", 50,
                     "The maximum total input sequence length after WordPiece tokenization. "
                     "Sequences longer than this will be truncated, and sequences shorter "
                     "than this will be padded. Must match data generation.")
flags.DEFINE_integer("preference_size", 11,
...
flags.DEFINE_string("gpu", '3', "gpu use.")
...
if __name__ == "__main__":
    ...
    print('checkpointDir:', FLAGS.checkpointDir)  # 自动保存模型的地址
    ...
    for input_pattern in FLAGS.train_input_file.split(","):
        train_input_files.extend(tf.io.gfile.glob(input_pattern)) # 得到train data
    ...
    if FLAGS.test_input_file is None:
        ...
        for input_pattern in FLAGS.test_input_file.split(","):
            test_input_files.extend(tf.io.gfile.glob(input_pattern)) # 得到test data
