...
class AbstractTrainer(object):
    ...
    def evaluate(self, eval_data, train_data):
...
class Trainer(AbstractTrainer):
    ...
    def _valid_epoch(self, valid_data,train_data, show_progress=False,):
        ...
        Returns:
            ...
            dict: valid result
        """
        valid_result = self.evaluate(valid_data,train_data, load_best_model=False, show_progress=show_progress)
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_score, valid_result = self._valid_epoch(valid_data, train_data, show_progress=show_progress)
    ...
    def _neg_sample_batch_eval(self, batched_data):
        ...
        if self.config['eval_type'] == EvaluatorType.VALUE:
            ...
            row_idx = row_idx.type(torch.long)
            col_idx = col_idx.type(torch.long)
    ...
    def evaluate(self, eval_data,train_data, load_best_model=True, model_file=None, show_progress=False ):
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            # print(positive_u.tolist())
        self.eval_collector.model_collect(self.model)
        struct = self.eval_collector.get_data_struct()
        seq_len = train_data.dataset.user_counter
        result = self.evaluator.evaluate(struct, seq_len)
