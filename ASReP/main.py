...
parser.add_argument('--aug_mode', default="seq2seq", help="data augment type, seq2seq on seq2point", type=str)
...
aug_data_signature = './aug_data/{}/lr_{}_maxlen_{}_hsize_{}_nblocks_{}_drate_{}_l2_{}_nheads_{}_gen_num_{}_M_{}'.format(
    ...
    args.num_heads, args.reversed_gen_number, args.M)
print(aug_data_signature)

model_signature = 'lr_{}_maxlen_{}_hsize_{}_nblocks_{}_drate_{}_l2_{}_nheads_{}_gen_num_{}'.format(args.lr, args.maxlen,
                                                                                                   args.hidden_units,
                                                                                                   args.num_blocks,
                                                                                                   args.dropout_rate,
                                                                                                   args.l2_emb,
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if (epoch % 20 == 0 and epoch >= 200) or epoch == args.num_epochs:
            ...
            if not (args.reversed == 1):
                ...
                t_test, t_test_short_seq, t_test_medium30_seq, t_test_medium40_seq, t_test_long_seq = evaluate(model, dataset, args, sess, "test")
                print(' testall: ' + str(t_test) + '\n')
                with open("Yelp_log.txt", 'a') as f:
                    f.write(' testall: ' + str(t_test) + '\n')
                # t_valid = evaluate(model, dataset, args, sess, "valid")
                ...
                print('epoch: '+str(epoch)+' testmedium30: ' + str(t_test_medium30_seq))
                print('epoch: '+str(epoch)+' testmedium40: ' + str(t_test_medium40_seq))
                print('epoch: '+str(epoch)+' testlong: ' + str(t_test_long_seq))
                print('epoch: '+str(epoch)+' testlong: ' + str(t_test))
                ...
                with open("Yelp_log.txt", 'a') as f:
                    f.write(' reversed_model got '+ '\n')
                print('epoch: ' + str(epoch) + ' test: ' + str("pretrain finished"))
...
if args.reversed == 1:
    if args.reversed_gen_number > 0:
        if not os.path.exists('./reversed_models/' + args.dataset + '_reversed/'):
            os.makedirs('./reversed_models/' + args.dataset + '_reversed/')
        saver.save(sess, './reversed_models/' + args.dataset + '_reversed/' + model_signature + '.ckpt')
    if args.aug_mode is "seq2seq":
        augmented_data = data_augment(model, dataset, args, sess, args.reversed_gen_number)
    elif args.aug_mode is "seq2point":
        augmented_data = data_augment_point(model, dataset, args, sess, args.reversed_gen_number)
    else:
        augmented_data = data_augment_inspire(model, dataset, args, sess, args.reversed_gen_number, model_signature)
